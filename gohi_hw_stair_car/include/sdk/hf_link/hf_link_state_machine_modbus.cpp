...
void StateMachineModbus::Request16(const HFMessageModbus* tx_message_, unsigned short int len){
    ...
    if(tx_message_->slave_addr==0x7F) //读卡器     
    {
    ...
    else   if(tx_message_->slave_addr==0x03) //电机3位置控制
    {
        tx_buffer[4]=0x00; //regNum=4; //2 bytes
        tx_buffer[5]=len/2;
        tx_buffer[6]=len;
...
void StateMachineModbus::Request04ToPAD(const HFMessageModbus* tx_message_){
    ...
    tx_buffer_length =18;
    // std::cerr <<"write_to_reg_data_ascii1  " <<(short int)tx_buffer_to_pad[0]<<std::endl; //*

    // std::cerr <<"write_to_reg_data_ascii2  " <<(short int)tx_buffer_to_pad[1]<<std::endl; 
    // std::cerr <<"write_to_reg_data_ascii3  " <<(short int)tx_buffer_to_pad[2]<<std::endl;

    // std::cerr <<"write_to_reg_data_ascii4  " <<(short int)tx_buffer_to_pad[3]<<std::endl; 
    // std::cerr <<"write_to_reg_data_ascii5  " <<(short int)tx_buffer_to_pad[4]<<std::endl; 
    
    // std::cerr <<"write_to_reg_data_ascii6  " <<(short int)tx_buffer_to_pad[5]<<std::endl; 
    // std::cerr <<"write_to_reg_data_ascii7  " <<(short int)tx_buffer_to_pad[6]<<std::endl; 
    
    // std::cerr <<"write_to_reg_data_ascii8  " <<(short int)tx_buffer_to_pad[7]<<std::endl; 
    // std::cerr <<"write_to_reg_data_ascii9  " <<(short int)tx_buffer_to_pad[8]<<std::endl; 
    
    // std::cerr <<"write_to_reg_data_ascii10  " <<(short int)tx_buffer_to_pad[9]<<std::endl; 
    // std::cerr <<"write_to_reg_data_ascii7  " <<(short int)tx_buffer_to_pad[10]<<std::endl; 
    
    // std::cerr <<"write_to_reg_data_ascii8  " <<(short int)tx_buffer_to_pad[11]<<std::endl; 
    // std::cerr <<"write_to_reg_data_ascii9  " <<(short int)tx_buffer_to_pad[12]<<std::endl; 
    
    // std::cerr <<"write_to_reg_data_ascii10  " <<(short int)tx_buffer_to_pad[13]<<std::endl; 
    // std::cerr <<"write_to_reg_data_ascii7  " <<(short int)tx_buffer_to_pad[14]<<std::endl; 
    
    // std::cerr <<"write_to_reg_data_ascii8  " <<(short int)tx_buffer_to_pad[15]<<std::endl; 
...
void StateMachineModbus::Request16ToPAD(const HFMessageModbus* tx_message_){
    ...
    unsigned char tx_buffer_to_pad[19];

       std::cerr <<"command6_____" <<std::endl;//setup 
    ...
    //LongSen=tx_message_->data[0]*2;
    
    
    tx_buffer[4]=tx_message_->data[1]; //regNum=4; //2 bytes
    tx_buffer[5]=tx_message_->data[0];
    tx_buffer[6]=tx_message_->data[0]*2;

    tx_buffer[7]=tx_message_->data[3]; //positionPhaseChange=500*10;// 2 bytes
    tx_buffer[8]=tx_message_->data[2]; 
    
    tx_buffer[9]=tx_message_->data[5];  //positiontype=0;// 2 bytes
    tx_buffer[10]=tx_message_->data[4];
    
    tx_buffer[11]=tx_message_->data[7];//position=0xfff;// 4 bytes
    tx_buffer[12]=tx_message_->data[6];

    tx_buffer[13]=tx_message_->data[9];
    tx_buffer[14]=tx_message_->data[8];
    ...
    HexToAscii(tx_buffer,&tx_buffer_to_pad[1],17);
    ...
    // tx_buffer_length =8;
        ...
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[0]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[1]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[2]<<std::endl;
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[3]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[4]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[5]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[6]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[7]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[8]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[9]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[10]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[11]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[12]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[13]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[14]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[15]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[16]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[17]<<std::endl; 
        std::cerr <<"read_from_reg_data1  " <<(short int)tx_buffer_to_pad[18]<<std::endl; 
...
// void StateMachineModbus::HexToAscii(unsigned char  hex, unsigned char &hex_HToAscii,unsigned char &hex_LToAscii)
//  {

//         hex_HToAscii = (hex& 0xF0) >> 4;  
//         hex_LToAscii = hex& 0x0F;   
//         if (hex_HToAscii < 10)                hex_HToAscii += 0x30;   
//         else  
//         {          
//             if (hex_HToAscii < 16)            hex_HToAscii= hex_HToAscii - 10 + 'A';
//         } 
//         if (hex_LToAscii < 10)                hex_LToAscii += 0x30;   
//         else  
//         {          
//             if(hex_LToAscii < 16)           hex_LToAscii = hex_LToAscii- 10 + 'A';
//         } 
        
// }
    
// void StateMachineModbus::HexToAscii(unsigned short int hex, unsigned char &hex_HToAscii,unsigned char &hex_LToAscii)
//  {
