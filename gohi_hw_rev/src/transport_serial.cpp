...
TransportSerial::TransportSerial():Transport("serial:///dev/ttyUSB2")
{
...
TransportSerial::TransportSerial(std::string url) : Transport(url)
{
...
void TransportSerial::start_a_write()
{
    ...
    if (!write_buffer_.empty())
    {
        boost::asio::async_write(*port_, 
                                 ...
                                 boost::bind(&TransportSerial::writeHandler, 
									         this, 
									         boost::asio::placeholders::error
								            ));
        write_buffer_.pop();
