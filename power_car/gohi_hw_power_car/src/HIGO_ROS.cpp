...
HIGO_ROS::HIGO_ROS(ros::NodeHandle &nh, std::string url, std::string config_addr) :
	...
	void HIGO_ROS::mainloop()
	{
		...
		while (ros::ok())
		{
		    ...
		    higo_ap_.updateCommand(READ_MOT5_SPEED, count,0);
			higo_ap_.updateCommand(READ_MOT6_SPEED, count,0);
		    
			higo_ap_.updateCommand(READ_MOT5_REAL_POSITION, count,0);		//读电机左轮的速度	
		    higo_ap_.updateCommand(READ_MOT6_REAL_POSITION, count,0);	 	//读电机右轮的速度	 
